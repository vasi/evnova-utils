...
@cmds = (
	...
	find		=> [\&find, '[-i] TYPE FIELDSPEC VALSPEC',
		...
		'Pass -- as the field to match against the whole resource'],
	rank		=> [\&rank, '[-r] [-m MAX] TYPE FIELD [FILT]',
		'sort resources by field value',
		'Flag --reverse sorts up, instead of down',
		'Flag --max sets the number of items to show'],
	...
	0 => 'Missions',
	misn		=> [\&misn, '[-lqdvs] [-p PILOT] [SPECSET]', 'show mission details',
		'Flag --level sets verbosity, from 0 to 4',
		'Flag --quiet is equivalent to -l0, --details to -l3, --verbose to -l4',
	...
	avail		=> [\&availMisns, '[-vufirlpb1s] PILOT [PROGRESS]',
		...
		'Flag --bar only shows missions available from bars',
		'Flag --random|-1 shows only a single random mission',
		'Flag --secret uses a fake identifier instead of misn ID',
	...
	dist		=> [\&showDist, '[-j JUMPS] SYST1 SYSY2',
		'find shortest path between systems',
		'With -j, find shortest number of segments'],
	...
	teleport => [\&setSpob, 'PILOT SPOB', "change the pilot's location"],
	unexplored => [\&unexplored, 'PILOT GOVT',
		'show which systems of a govt are unexplored'],
